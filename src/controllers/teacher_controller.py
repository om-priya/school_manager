...
class TeacherController:
    def get_all_teacher(self):
        try:
            ...
            return SuccessResponse(200, "Here's the list of Teachers", res_data)
        except DataNotFound:
            return abort(404, message=ErrorResponse(404, "No Teachers Found"))

    def get_single_teacher(self, teacher_id):
        try:
            res_data = TeacherHandler().get_teacher_by_id(teacher_id)
            return SuccessResponse(200, "Here's the details of Teacher", res_data)
        except DataNotFound:
            return abort(404, message=ErrorResponse(404, "No Teacher Found"))

    def approve_teacher(self, teacher_id):
        try:
            TeacherHandler().approve_teacher(teacher_id)
            return SuccessResponse(200, "Teacher Approved Successfully")
        except DataNotFound:
            return abort(404, message=ErrorResponse(404, "No Teacher Found"))
        except FailedAction:
            return abort(409, message=ErrorResponse(409, "Teacher Can't be Approved"))

    def delete_teacher(self, teacher_id):
        try:
            TeacherHandler().delete_teacher(teacher_id)
            return SuccessResponse(200, "Teacher Deletd")
        except DataNotFound:
            return abort(404, message=ErrorResponse(404, "No Teacher Found"))
