...
def test_handle_principal(monkeypatch, mock_super_admin_controller_obj, capsys):
...
def test_handle_staff(monkeypatch, mock_super_admin_controller_obj, capsys):
    options = ["1", "2", "3", "4", "ab", "5"]
    monkeypatch.setattr(builtins, "input", lambda *args: options.pop(0))
    mock_super_admin_controller_obj.handle_staff()
    assert mock_super_admin_controller_obj.staff_handler_obj.view_staff.call_count == 1
    assert (
        mock_super_admin_controller_obj.staff_handler_obj.create_staff.call_count == 1
    )
    assert (
        mock_super_admin_controller_obj.staff_handler_obj.update_staff.call_count == 1
    )
    assert (
        mock_super_admin_controller_obj.staff_handler_obj.delete_staff.call_count == 1
    )
    captured = capsys.readouterr()
