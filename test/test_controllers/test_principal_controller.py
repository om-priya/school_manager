...
def test_handle_teacher(monkeypatch, mock_principal_controller_obj, capsys):
    ...
    assert (
        mock_principal_controller_obj.teacher_handler_obj.update_teacher.call_count == 1
    )
    assert (
        mock_principal_controller_obj.teacher_handler_obj.delete_teacher.call_count == 1
    )
    captured = capsys.readouterr()
    assert "\nInvalid Input Enter Only [1-6]\n" in captured.out


def test_handle_feedbacks(monkeypatch, mock_principal_controller_obj, capsys):
    options = ["1", "2", "ab", "3"]
    monkeypatch.setattr(builtins, "input", lambda *args: options.pop(0))
    mock_principal_controller_obj.handle_feedbacks()
    assert (
        mock_principal_controller_obj.feedback_handler_obj.read_feedback.call_count == 1
    )
    assert (
        mock_principal_controller_obj.feedback_handler_obj.give_feedback.call_count == 1
    )
    captured = capsys.readouterr()
    assert "\nInvalid Input Enter Only [1-3]\n" in captured.out


def test_handle_events(monkeypatch, mock_principal_controller_obj, capsys):
    options = ["1", "2", "ab", "3"]
    monkeypatch.setattr(builtins, "input", lambda *args: options.pop(0))
    mock_principal_controller_obj.handle_events()
    assert mock_principal_controller_obj.event_handler_obj.read_event.call_count == 1
    assert mock_principal_controller_obj.event_handler_obj.create_event.call_count == 1
    captured = capsys.readouterr()
    assert "\nInvalid Input Enter Only [1-3]\n" in captured.out


def test_handle_leaves(monkeypatch, mock_principal_controller_obj, capsys):
    options = ["1", "2", "ab", "3"]
    monkeypatch.setattr(builtins, "input", lambda *args: options.pop(0))
    mock_principal_controller_obj.handle_leaves()
    assert (
        mock_principal_controller_obj.leave_handler_obj.see_leave_status.call_count == 1
    )
    assert mock_principal_controller_obj.leave_handler_obj.apply_leave.call_count == 1
    captured = capsys.readouterr()
    assert "\nInvalid Input Enter Only [1-3]\n" in captured.out


def test_view_issue(mock_principal_controller_obj):
    mock_principal_controller_obj.view_issues()
    assert mock_principal_controller_obj.issue_handler_obj.view_issue.call_count == 1
